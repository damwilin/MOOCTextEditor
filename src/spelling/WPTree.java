...
public class WPTree implements WordPath {
    ...
    public List<String> findPath(String word1, String word2) {
        ...
        while (!queue.isEmpty() && !visited.contains(word2)) {
            //Remove first node from the queue and assign it to curr
            WPTreeNode currNode = queue.remove(0);
            //Get a list of real word neighbours
            List<String> currNeighbors = nw.distanceOne(currNode.getWord(), true);
            for (String currWord : currNeighbors) {
                if (!visited.contains(currWord)) {
                    WPTreeNode currChild = currNode.addChild(currWord);
