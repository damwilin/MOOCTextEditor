...
public abstract class Document {
    ...
    protected int countSyllables(String word) {
        ...
        for (int i = 0; i < cArray.length; i++) {
            if (i == cArray.length - 1 && cArray[i] == 'e' && newSyllable && numOfSyllables > 0)
                numOfSyllables--;
    ...
    public double getFleschScore() {
        ...
        double fleshScore = 206.835 - 1.015 * ((double) getNumWords() / getNumSentences()) - 84.6 * ((double) getNumSyllables() / getNumWords());
        return fleshScore;
    }
}
